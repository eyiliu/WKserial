...
void SlowControlClient(Bool_t offline_mode = false, TString offlinefile="reallyall_data.txt", Double_t hours_width_of_plotting = 1){
  ...
  Double_t time, date_float, Hybrid1Temperature, HVVoltage, Hybrid2Temperature, HVCurrent, ChillerActualTemperature, ChillerTargetTemperature, ChuckTemperature, ChuckHumidity, ChuckDewpoint;
  Double_t prev_time, prev_date_float, prev_Hybrid1Temperature, prev_HVVoltage, prev_Hybrid2Temperature, prev_HVCurrent, prev_ChillerActualTemperature, prev_ChillerTargetTemperature, prev_ChuckTemperature, prev_ChuckHumidity, prev_ChuckDewpoint, average_slope;
  
  TCanvas *c1 = new TCanvas("c1","Slow Control Monitoring");
  ...
  TGraph *h_ChillerActualTemperature   = new TGraph();
  ...
  Int_t Col_h_ChillerActualTemperature = kGreen+1;
  ...
  if(!offline_mode)  dataout << "time, Hybrid1Temperature, HVVoltage, Hybrid2Temperature, HVCurrent, ChillerActualTemperature, ChillerTargetTemperature, ChuckTemperature, ChuckHumidity, ChuckDewpoint" << endl;
  ...
  for (Int_t i=0; 1; ++i) {
    ...
    if(!offline_mode) { // getting live data from devices on serial ports
      ...
      prev_ChillerActualTemperature = ChillerActualTemperature ;
      prev_ChillerTargetTemperature = ChillerTargetTemperature ;
      ...
      ChillerActualTemperature = mySpy->Request("Get_ChillerActualTemperature");
      if( ChillerActualTemperature < -300. ) ChillerActualTemperature = -99.99;
      ChillerTargetTemperature = mySpy->Request("Get_ChillerTargetTemperature");
      if( ChillerTargetTemperature < -300. ) ChillerTargetTemperature = -99.99;
      ...
      if( line.ReadLine(stream) ) { // still lines to read
        ...
        stream5 >> HVCurrent;
        stream5 >> ChillerActualTemperature;
        stream5 >> ChillerTargetTemperature;
        ...
        if( 42 != 42 ) {
          cout<<"We have read this: "
              ...
              <<Hybrid2Temperature	    <<" "
              <<HVCurrent		    <<" "
              <<ChillerActualTemperature<<" "
              <<ChillerTargetTemperature<<" "
              <<ChuckTemperature        <<" "
              <<ChuckHumidity	    <<" "
              <<ChuckDewpoint           <<" "
              <<endl;
...
//    if( (ChillerActualTemperature-5.) > ChuckDewpoint )  {
	...
	//      Col_h_ChillerActualTemperature = kGreen+1; 
...
//    if( ChillerActualTemperature < -50 ) {
//      c1->SetFillColor(kBlue);
//      Col_h_ChillerActualTemperature = kBlue; 
...
//    }
    ...
    h_ChillerActualTemperature->SetPoint(nlines-1, time, ChillerActualTemperature);
    ...
    if( !offline_mode || !(nlines % 10000) || fileendreached ) {
      ...
      c1->cd(5);
      h_ChillerActualTemperature->SetLineColor(kBlue);
      if(offline_mode) {
        h_ChillerActualTemperature->SetLineStyle(1);
        h_ChillerActualTemperature->SetMarkerStyle(6);
        h_ChillerActualTemperature->SetMarkerColor(kBlue);
        h_ChillerActualTemperature->Draw("AP");
      }
      else{
        if(42 == 42) {       
          axis = h_ChillerActualTemperature->GetXaxis();
          axis->SetLimits(old_time.Convert(), current_time.Convert()); 
        }
        h_ChillerActualTemperature->Draw("AL");
      }
      gPad->Modified();
      ...
      if( TMath::Abs(HVVoltage) < 1. ) {     
	CurrentValue_1_1.Append( Form("%2.1g", HVVoltage ) ); 
      }
      else {      
	CurrentValue_1_1.Append( Form("%3.2f", HVVoltage ) );
      }
      ...
      c1->cd(5);
      //    h_ChillerActualTemperature->SetTitle("ChillerActualTmp");//Temperature of actual chilling liquid temperature
      h_ChillerActualTemperature->GetXaxis()->SetTitle("time(GMT)");
      h_ChillerActualTemperature->GetYaxis()->SetTitle("Chiller Temperature (#circC)");
      h_ChillerActualTemperature->GetXaxis()->SetTimeDisplay(1);
      h_ChillerActualTemperature->GetXaxis()->SetTimeFormat("%Y-%m-%d -- %H:%M:%S");
      h_ChillerActualTemperature->GetXaxis()->SetTimeOffset(0,"gmt");    
      TString CurrentValue_3 = "";
      if ( ChillerTargetTemperature > -50. ) {
        CurrentValue_3.Append( Form("%2.1f", ChillerActualTemperature ) );
      }
      else {
        CurrentValue_3.Append( "UNDEF " );
      }
      CurrentValue_3.Append("#circC Chiller Temp (Target: ");
      if ( ChillerTargetTemperature > -50. ) {
        CurrentValue_3.Append( Form("%2.1f", ChillerTargetTemperature ) );
      }
      else {
        CurrentValue_3.Append( " UNDEF " );
      }
      CurrentValue_3.Append("#circC, Slope ");
      average_slope = approxRollingAverage(average_slope,60.*(ChillerActualTemperature-prev_ChillerActualTemperature)/(time-prev_time));
      if(i>30) {
	CurrentValue_3.Append( Form("%2.2f", average_slope ) );
      }
      else {
	CurrentValue_3.Append( Form("%2.2f",  60.*(ChillerActualTemperature-prev_ChillerActualTemperature)/(time-prev_time) ) );
      }
      CurrentValue_3.Append("#circC/min, ");
      if((60.*(ChillerActualTemperature-prev_ChillerActualTemperature)/(time-prev_time))) {     
//	CurrentValue_3.Append( Form("%2.2f",  TMath::Abs((ChillerActualTemperature-ChillerTargetTemperature)/(60.*(ChillerActualTemperature-prev_ChillerActualTemperature)/(time-prev_time))) ) ); 
	CurrentValue_3.Append( Form("%2.2f",  TMath::Abs((ChillerActualTemperature-ChillerTargetTemperature)/average_slope) ) ); 
      }
      else {
	CurrentValue_3.Append(" UNDEF"); 
      }
      ...
      lable_3->SetTextColor(Col_h_ChillerActualTemperature);
      lable_3->Draw();
      gPad->Modified();
...
//        gSystem->Exec("copy current_SC.svg H\:\\www\\SlowControl");
      ...
      dataout << date_root.GetDate()<<"-"<<date_root.GetHour()<<":"<<date_root.GetMinute()<<":"<<date_root.GetSecond()<<" "<< Hybrid1Temperature <<" "<< HVVoltage <<" "<< Hybrid2Temperature <<" "<< HVCurrent <<" "<< ChillerActualTemperature <<" "<< ChillerTargetTemperature <<" "<< ChuckTemperature <<" "<< ChuckHumidity <<" "<< ChuckDewpoint << endl;
